...
ALTER TABLE public.registrations
  ...
  ADD COLUMN IF NOT EXISTS mentor_branch text;
...
DECLARE
  ...
  v_mentor jsonb := payload->'mentor';
  v_mentor_name text := NULLIF(btrim(coalesce(v_mentor->>'name','')),'');
  v_mentor_email text := NULLIF(lower(btrim(coalesce(v_mentor->>'email',''))),'');
  v_mentor_phone text := NULLIF(btrim(coalesce(v_mentor->>'phone','')),'');
  v_mentor_branch text := NULLIF(btrim(coalesce(v_mentor->>'branch','')),'');
...
BEGIN
  ...
  IF v_mentor_name IS NULL OR v_mentor_email IS NULL OR v_mentor_phone IS NULL OR v_mentor_branch IS NULL THEN
    RAISE EXCEPTION 'invalid_mentor_details' USING ERRCODE='P0001';
  END IF;
  IF v_mentor_email !~* '^[^@\s]+@[^@\s]+\.[^@\s]+$' THEN
    RAISE EXCEPTION 'invalid_mentor_email' USING ERRCODE='P0001';
  END IF;
  IF v_mentor_phone !~ v_phone_pattern THEN
    RAISE EXCEPTION 'invalid_mentor_phone' USING ERRCODE='P0001';
  END IF;
  ...
  INSERT INTO public.registrations(team_id, event_id, status, registration_code, qr_token, email_status,
    ...
    mentor_name, mentor_email, mentor_phone, mentor_branch)
  VALUES (v_team_id, v_event_id, 'confirmed', v_reg_code, v_qr_token, 'queued',
    v_idea_title, v_abstract, v_project_track,
    v_mentor_name, v_mentor_email, v_mentor_phone, v_mentor_branch)
  ...
  VALUES (v_leader_email, 'SPARK TANK 4.0 — Registration Confirmed', 'registration', 'pending',
    jsonb_build_object(
      ...
      'mentor_name', v_mentor_name, 'mentor_email', v_mentor_email));
