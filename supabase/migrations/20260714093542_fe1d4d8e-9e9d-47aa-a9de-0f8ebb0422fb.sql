-- Add idea/abstract/project_type support to registrations
DO $$ BEGIN
  CREATE TYPE public.project_track AS ENUM ('software','hardware');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

ALTER TABLE public.registrations
  ADD COLUMN IF NOT EXISTS idea_title text,
  ADD COLUMN IF NOT EXISTS abstract text,
  ADD COLUMN IF NOT EXISTS project_track public.project_track;
...
BEGIN
  ...
  IF v_project_track IS NULL OR v_project_track NOT IN ('software','hardware') THEN
  ...
  VALUES (v_team_id, v_event_id, 'confirmed', v_reg_code, v_qr_token, 'queued',
    v_idea_title, v_abstract, v_project_track::public.project_track)
